          ...
          <div class="widget-content nopadding">
            ...
            <form method="post" action="{{ url('/deleteMultiple') }}">
              ...
              <table class="table table-bordered table-striped with-check">
                ...
                <tbody>
                  @foreach($brandData as $ar)
                  <tr>
                    <td><input type="checkbox" name="adminMdelete[]" class="checbox" id="select_all" value="{{ $ar->brand_id }}" /></td>
                    <td>{{ $ar->brand_cat_id }}</td>
                    <td>{{ $ar->brand_sub_id }}</td>
                    <td>{{ $ar->brand_extra_id }}</td>
                    <td>{{ $ar->brand_name }}</td>
                    <td>
                        @if($ar->brand_status == 0) 
                            <a href='javascript:active_brand("{{ $ar->brand_id }}")' class="btn btn-danger">Deactive</a>

                        @else    
                            <a href='javascript:deactive_brand("{{ $ar->brand_id }}")' class="btn btn-success">Active</a>

                        @endif
                    </td>
                    <td>{{ $ar->created_at }}</td>
                    <td>{{ $ar->updated_at }}</td>
                    <td><a href='{{ url("/BrandDelete/$ar->brand_id")}}'><i class="icon-trash"></i></a> | <a href='{{ url("/BrandEdit/$ar->brand_id")}}'><i class="icon-edit"></i></a></td>
                  </tr>
                  @endforeach
                  <tr>
                    <td colspan="9">
